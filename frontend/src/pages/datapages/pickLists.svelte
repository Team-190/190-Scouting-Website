<script>
    ...
    async function oprFill() {
        if (!eventCode) { alert("Please select an event first."); return; }
    ...
    function importPicklists() {
        if (!importData) { alert("Please paste the data to import."); return; }
        try {
    ...
    function handleDrop(targetListKey) {
        ...
        if (sourceList === targetListKey) { draggedItem = null; return; }
    ...
    function handleDropToRemove(event) {
        if (!draggedItem) return;
        if (event.target.closest(".picklist .list") || event.target.closest(".alliance-list .list")) return;
    ...
    function handleDropOnAlliance(targetAllianceId) {
        ...
        if (!targetAlliance) { draggedItem = null; return; }
    ...
    async function updateAllianceCaptains() {
        ...
        for (let i = emptyIndex; i < 7; i++) alliances[i].teams = alliances[i + 1].teams;
        const occupied = new Set(alliances.flatMap((a) => a.teams.map((t) => t.team_number)));
    ...
    function createAllianceSelection() {
        ...
        if (!name) { alert("Please enter a name for the new alliance selection."); return; }
        ...
        allianceSelections[newId] = { name, alliances: makeEmptyAlliances(), isFourTeamAlliance: false };
    ...
    function createAndSwitchToNewAllianceSelection(name) {
        ...
        const newId = `selection_${Date.now()}`;
        allianceSelections[newId] = { name: newName, alliances: makeEmptyAlliances(), isFourTeamAlliance };
        activeAllianceSelectionId = newId;
    }
    ...
    function pasteAllianceSelection() {
        if (!allianceImportData) { alert("Please paste the alliance selection data into the text box."); return; }
        try {
            ...
            const newId = `selection_${Date.now()}`;
            allianceSelections[newId] = { name: newName, alliances: newAlliances, isFourTeamAlliance: isFourTeamStr === "1" };
            activeAllianceSelectionId = newId;
            allianceImportData = "";
        } catch (error) {
                        ...
                        {#each Object.entries(picklists) as [key, list]}
                            <div class="picklist">
                                <h2>
                                    ...
                                    <button onclick={() => deletePickList(key)} style="background: transparent; border: none; font-size: 1.2rem; padding: 0;">X</button>
                                </h2>
                                <div
                                    class="list"
                                    role="application"
    ...
    {#if activeView === "alliances"}
        <div class="bottom-bar">
            <div class="alliance-management">
                ...
                <button onclick={deleteAllianceSelection} disabled={Object.keys(allianceSelections).length <= 1 || activeAllianceSelectionId === "default"}>Delete</button>
...
<style>
    ...
    .header-section { text-align: center; padding: 10px 0; margin-bottom: 10px; }
    .header-section h1 {
    ...
    .tabs { display: flex; border: 2px solid var(--frc-190-red); border-radius: 6px; overflow: hidden; }
    .tabs button { border-radius: 0; border: none; background: #222; color: #aaa; padding: 10px 20px; font-size: 16px; }
    ...
    .main-content { display: flex; width: 100%; gap: 5px; align-items: flex-start; }
    ...
    .team-list { flex: 1; overflow-y: auto; padding: 10px; min-height: 0; }

    .view-container { flex: 1; min-width: 0; min-height: 0; }
    .picklist-view { height: 75vh; overflow-y: auto; padding-right: 10px; padding-bottom: 20px; }

    .container { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 20px; margin-top: 20px; }
    ...
    .picklist h2 input { background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.3); color: white; width: 70%; }
    .picklist .list, .alliance-list .list { flex: 1; padding: 10px; background: #252525; }
    ...
    .fixed-buttons { position: fixed; bottom: 100px; right: 30px; display: flex; flex-direction: column; gap: 10px; z-index: 50; }
    ...
    .alliances-container { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; max-width: 100%; }
    @media (max-width: 1600px) { .alliances-container { grid-template-columns: repeat(2, 1fr); } }
    ...
    .picklists-sidebar h2 { background: var(--frc-190-red); color: white; margin: 0; padding: 10px; text-align: center; font-size: 1.2rem; flex-shrink: 0; }
    .picklists-scroll { flex: 1; overflow-y: auto; padding: 10px; min-height: 0; }

    .sidebar-picklist { background: #252525; border: 1px solid #333; border-radius: 6px; margin-bottom: 15px; overflow: hidden; }
    .sidebar-picklist h3 { background: #2d2d2d; color: var(--frc-190-red); margin: 0; padding: 8px 12px; font-size: 1rem; border-bottom: 1px solid #333; }
    .sidebar-list { padding: 5px; }
    .sidebar-team {
        display: flex; justify-content: center; align-items: center;
        padding: 3px; background: #1a1a1a; border: 1px solid #333;
        border-radius: 4px; margin-bottom: 2px; color: white;
        cursor: grab; transition: all 0.15s; font-size: 0.8rem; font-weight: bold;
    }
    ...
    .bottom-bar {
        ...
        background: #151515; border-top: 2px solid var(--frc-190-red);
        padding: 15px 30px; z-index: 100; display: flex;
