<script>
  ...
  let allianceSelections = $state(
    loadFromStorage("allianceSelections", {
      default: { name: "Default", alliances: makeEmptyAlliances() },
    }),
  );
  ...
  let rawTeamViewData = $state([]);

  $effect(() => {
    if (eventCode) getTeams();
    const stored = localStorage.getItem("data");
    rawTeamViewData = stored ? JSON.parse(stored) : [];
  });
  ...
  $effect(() => {
    if (!isFourTeamAlliance) {
      alliances.forEach((alliance) => {
        if (alliance.teams.length > 3) alliance.teams.length = 3;
      });
    }
  });
  $effect(() => {
    ...
    if (stored) {
      try {
        ...
        rawTeamViewData = parsed.map((row) => {
          const flat = {};
          for (const key of Object.keys(row)) {
            const val = row[key];
            flat[key] = Array.isArray(val) && val.length === 2 ? val[1] : val;
          }
          return flat;
        });
      } catch {
  ...
  function onTeamMouseEnter(team, el) {
  ...
  async function rankFill() {
    ...
    if (rankedTeams.length === 0) {
      alert("Please ensure team rankings are loaded.");
      return;
    }
    const occupied = () =>
    ...
    for (let i = 7; i >= 0; i--) pickNext(alliances[i]);
    if (isFourTeamAlliance) for (let i = 7; i >= 0; i--) pickNext(alliances[i]);
  }
  ...
  async function oprFill() {
    ...
    if (!alliances.every((a) => a.teams.length > 0)) {
      alert("Could not populate all alliance captains.");
      return;
    }
        ...
        >
          {#each getAvailableTeamNumbers() as teamNumber (teamNumber)}
            ...
            <div
              ...
              onmouseenter={(e) => onTeamMouseEnter(team, e.currentTarget)}
                ...
                >
                  {#each list.teams as team (team.team_number)}
                    <div
                      style="pointer-events: {isDragging ? 'none' : 'auto'};"
                      onmouseenter={(e) =>
                        onTeamMouseEnter(team, e.currentTarget)}
                      onmouseleave={onTeamMouseLeave}
                    >
                      <Team
                        {team}
                  ...
                  >
                    ...
                    >
                      {#each alliance.teams as team (team.team_number)}
                        <div
                          style="pointer-events: {isDragging
                            ? 'none'
                            : 'auto'};"
                          onmouseenter={(e) =>
                            onTeamMouseEnter(team, e.currentTarget)}
