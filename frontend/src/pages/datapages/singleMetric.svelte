<script lang="ts">
  ...
  function getQuintileBg(p: number): string { return QUINTILE_COLORS[p] ?? "#4D4D4D"; }
  function getQuartileBg(p: number): string { return QUARTILE_COLORS[p] ?? "#4D4D4D"; }
  ...
  function getClimbStateColor(climbState: any, attemptClimb: any): string {
    if (climbState === null || climbState === undefined || climbState === "" || climbState === -1) return "#808080";
  ...
  function computeGlobalStats(values: number[]): typeof globalStats {
    if (!values.length) return { mean: 0, sd: 0, p25: 0, p50: 0, p75: 0, isNumeric: true };
  ...
  function assignAlexPercentiles(rows: any[]): any[] {
    ...
    return rows.map((row) => {
      ...
      const rank = (validRows.length - validRows.indexOf(row) - 1) / validRows.length;
      const alexPercentile = rank < 0.2 ? 0 : rank < 0.4 ? 20 : rank < 0.6 ? 40 : rank < 0.8 ? 60 : 80;
  ...
  function buildGrid() {
    ...
    rowData = availableTeams.map((team) => {
      ...
      if (isBooleanMetric) {
        const countTrue = (row) => qLabels.filter((q) => booleanTrue(row[q])).length;
        const diff = countTrue(b) - countTrue(a);
        return diff !== 0 ? diff : a.team.localeCompare(b.team);
      }
      if (!isNumericMetric || isClimbStateMetric) return a.team.localeCompare(b.team);
      if (inverted) {
  ...
  async function buildEFSGrid() {
    ...
    const efsCellStyle = (v: any) => {
      if (v === null || v === undefined) return { background: "#333", color: "white", fontWeight: 600, fontSize: "16px", textAlign: "center", border: "1px solid #555" };
      if (v === 0) return { background: "black", color: "white", fontWeight: 600, fontSize: "18px", textAlign: "center" };
    ...
    const efsStatStyle = (v: any, border?: string) => {
      if (v === null || v === undefined) return statCellStyle("#4D4D4D", "white", border);
    ...
    const columnDefs = [
      ...
      {
        headerName: "Med.", field: "median", flex: 1, minWidth: 80,
        headerClass: "header-center", cellClass: "cell-center",
    ...
    const columnDefs = [
      makeTeamColumnDef(),
      {
        ...
        cellStyle: (params) => {
          ...
          if (v === null || v === undefined) return statCellStyle("#4D4D4D", "white");
  ...
  async function onAutoOnlyChange() {
    ...
    try {
      ...
      if (!availableTeams.length) { error = "No team data found."; loading = false; return; }
      metrics = computeMetrics();
      if (!metrics.length) { error = "No metrics found."; loading = false; return; }
  ...
  function addChart(type: string) {
    const chart: any = {
      id: uuidv4(), type, el: null, instance: null,
      selectedTeams: new Set(availableTeams),
      showFilter: false, showMetricFilter: false,
  ...
  function toggleChartTeam(chart: any, team: string) {
    chart.selectedTeams.has(team) ? chart.selectedTeams.delete(team) : chart.selectedTeams.add(team);
  ...
  function toggleChartMetric(chart: any, metric: string) {
    ...
    chart.selectedMetrics.has(metric) ? chart.selectedMetrics.delete(metric) : chart.selectedMetrics.add(metric);
  ...
  function selectChartAllMetrics(chart: any) {
    chart.selectedMetrics = new Set(metrics.filter((m) => checkIsNumericMetric(m)));
    updateChartDataset(chart);
    charts = charts;
  }

  function updateAllCharts() { charts.forEach((c) => updateChartDataset(c)); }
  ...
  function updateChartDataset(chart: any) {
    ...
    if (!isNumericData && chart.type !== "pie" && chart.type !== "radar") {
      option = {
        title: { text: "This chart requires numeric data.", left: "center", top: "center", textStyle: { color: "#fff", fontSize: 16 } },
  ...
  function getBarOption(filterSet: Set<string>) {
    ...
    return {
      ...
      xAxis: { type: "category", data: filtered.map((r) => r.team), axisLabel: { ...axisLabelStyle, interval: 0, rotate: 90 } },
      yAxis: { type: "value", name: selectedMetric, axisLabel: axisLabelStyle, nameTextStyle: axisNameStyle },
      series: [{ data: filtered.map((r) => r.mean), type: "bar", name: selectedMetric, itemStyle: { color: "#C81B00" }, label: { show: true, color: "#ffffff", rotate: 90, offset: [20, 0] } }],
    };
  }
  ...
  function getLineOption(filterSet: Set<string>) {
    ...
    return {
      ...
      xAxis: { type: "category", data: filtered.map((r) => r.team), axisLabel: { ...axisLabelStyle, interval: 0, rotate: 90 } },
      yAxis: { type: "value", name: selectedMetric, axisLabel: axisLabelStyle, nameTextStyle: axisNameStyle },
      series: [{ data: filtered.map((r) => r.mean), type: "line", name: selectedMetric, lineStyle: { color: "#C81B00" }, itemStyle: { color: "#C81B00" }, label: { show: true, color: "#ffffff", rotate: 90, offset: [20, 0] } }],
    };
  }
  ...
  function getPieOption(filterSet: Set<string>, isNumericData: boolean) {
    let data;
    if (isNumericData) {
      data = rowData.filter((r) => filterSet.has(r.team)).map((r) => ({ value: r.mean, name: r.team }));
    } else {
      const counts: Record<string, number> = {};
      availableTeams.filter((t) => filterSet.has(t)).forEach((team) => {
    ...
    return {
      ...
      series: [{ type: "pie", data, name: selectedMetric, radius: "60%", label: { color: "#ffffff" } }],
    };
  }

  function getScatterOption(filterSet: Set<string>) {
    const teams = rowData.filter((r) => filterSet.has(r.team)).map((r) => r.team);
    ...
    return {
      tooltip: { trigger: "item" },
      xAxis: { type: "category", data: teams, name: "Team", axisLabel: { ...axisLabelStyle, interval: 0, rotate: 90 } },
      yAxis: { type: "value", name: selectedMetric, axisLabel: axisLabelStyle, nameTextStyle: axisNameStyle },
      series: [{ symbolSize: 10, data: scatterData, type: "scatter", itemStyle: { color: "#C81B00" }, label: { show: true, color: "#ffffff" } }],
    };
  }
  ...
  function getRadarOption(chart: any) {
    ...
    const maxValues = numericMetrics.map((m) => {
      ...
      selectedTeams.forEach((team) => {
        (teamData[team] ?? []).forEach((r) => {
          const v = r[key];
          if (isNumeric(v)) { const num = Number(v); if (num !== 0 && num !== -1 && num > max) max = num; }
        });
      });
      return max;
    });
    ...
    const seriesData = selectedTeams.map((team, idx) => {
      ...
      const avgValues = numericMetrics.map((m) => {
        const key = resolveDataKey(m);
        const values = rows.map((r) => r[key]).filter((v) => isNumeric(v) && Number(v) !== 0 && Number(v) !== -1).map(Number);
    ...
    return {
      ...
      radar: {
        indicator: numericMetrics.map((m, i) => ({ name: `${m.replaceAll("_", " ")} (${formatMaxValue(maxValues[i])})`, max: maxValues[i] })),
        splitNumber: 4,
      ...
      {#each charts as chart (chart.id)}
        <div class="chart-wrapper">
          <div class="chart-controls">
            <button class="mini-btn" on:click={() => { chart.showFilter = !chart.showFilter; charts = charts; }}>
            ...
            {#if chart.type === "radar"}
              <button class="mini-btn" on:click={() => {
                if (!chart.showMetricFilter) chart.selectedMetrics = new Set(metrics.filter((m) => checkIsNumericMetric(m)));
            ...
            <button class="mini-btn" on:click={() => removeChart(chart.id)} style="border-color: #666;">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          {#if chart.showFilter}
            <div class="local-filter-panel">
              <div class="local-filter-actions">
                <button class="mini-btn" on:click={() => selectChartAll(chart)}>Select All Teams</button>
              </div>
              <div class="local-grid">
                {#each availableTeams as team}
                  <label class="mini-checkbox">
                    <input type="checkbox" checked={chart.selectedTeams.has(team)} on:change={() => toggleChartTeam(chart, team)} />
                    {team}
                  </label>
                {/each}
              </div>
          ...
          {#if chart.type === "radar" && chart.showMetricFilter}
            <div class="local-filter-panel">
              <div class="local-filter-actions">
                <button class="mini-btn" on:click={() => selectChartAllMetrics(chart)}>Select All</button>
              </div>
              <div class="local-grid">
                {#each Array.from(new Set(metrics.filter((m) => checkIsNumericMetric(m)))) as metric (metric)}
                  <label class="mini-checkbox">
                    <input
                      ...
                      disabled={chart.selectedMetrics?.size <= 3 && chart.selectedMetrics?.has(metric)}
              ...
              <p style="font-size: 12px; color: #aaa; margin: 8px 0 0;">Selected: {chart.selectedMetrics?.size ?? 0}</p>
            </div>
          {/if}

          <div class="chart-container" bind:this={chart.el}></div>
          <p class="chart-label">{chart.type} Chart - {selectedMetric.replaceAll("_", " ")}</p>
        </div>
      {/each}
    </div>
  </div>
...
<style>
  ...
  .page-wrapper {
    ...
    padding: 20px; background: var(--wpi-gray);
  }
  ...
  :global(.ag-header-cell) { background: var(--frc-190-red) !important; color: white !important; font-size: 16px; font-weight: bold; }
  ...
  .controls {
    padding: 15px 25px; background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%);
    color: white; font-size: 18px; display: flex; gap: 30px;
    align-items: center; justify-content: center;
    width: 80%; max-width: 1200px; border-radius: 10px; margin-bottom: 20px;
    border: 2px solid var(--frc-190-red); box-shadow: 0 4px 15px rgba(0, 0, 0, 0.3);
  }
  ...
  select {
    ...
    color: white; font-size: 16px; border: 2px solid var(--frc-190-red);
  ...
  .grid-container { width: 80vw; background: var(--frc-190-black); border-radius: 8px; box-shadow: 0 8px 30px rgba(0, 0, 0, 0.4); }

  .graph-section { width: 80vw; margin-top: 30px; display: flex; flex-direction: column; align-items: center; padding-bottom: 50px; }
  ...
  .plus-btn {
    ...
    align-items: center; justify-content: center; font-size: 2rem; font-weight: 600;
    border: 2px solid var(--frc-190-red); background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%);
  ...
  .dropdown {
    ...
    border: 2px solid var(--frc-190-red); border-radius: 8px;
    list-style: none; padding: 0; margin: 0; width: 150px;
    box-shadow: 0 4px 15px rgba(0, 0, 0, 0.4); z-index: 10; overflow: hidden;
  }
  ...
  .charts-grid { display: flex; justify-content: center; flex-wrap: wrap; width: 100vw; }
  .chart-wrapper {
    position: relative; display: flex; flex-direction: column; align-items: stretch;
    width: 80vw; background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%);
    border: 2px solid var(--frc-190-red); border-radius: 8px; padding: 15px;
    box-shadow: 0 4px 15px rgba(0, 0, 0, 0.3); margin-bottom: 20px;
  }
  .chart-container { width: 100%; height: 350px; display: flex; justify-content: center; align-items: center; flex-grow: 1; background: rgba(255, 255, 255, 0.05); border-radius: 6px; }
  .chart-label { margin-top: 10px; font-weight: bold; text-transform: capitalize; text-align: center; color: white; font-size: 1rem; }

  .chart-controls { display: flex; justify-content: flex-end; gap: 10px; margin-bottom: 10px; }
  .mini-btn { background: transparent; border: 1px solid var(--frc-190-red); color: white; padding: 4px 10px; font-size: 14px; border-radius: 4px; cursor: pointer; display: flex; align-items: center; gap: 5px; }
  .mini-btn:hover { background: rgba(200, 27, 0, 0.2); }

  .local-filter-panel { background: rgba(0, 0, 0, 0.3); border: 1px solid #444; border-radius: 6px; padding: 10px; margin-bottom: 15px; max-height: 200px; overflow-y: auto; }
  .local-filter-actions { display: flex; gap: 10px; margin-bottom: 10px; padding-bottom: 5px; border-bottom: 1px solid #444; }
  .local-grid { display: flex; justify-content: space-between; flex-wrap: wrap; gap: 10px; }
  .mini-checkbox { font-size: 13px; color: #ddd; display: flex; align-items: center; gap: 4px; cursor: pointer; }
  .mini-checkbox input { accent-color: var(--frc-190-red); }
</style>