<script>
  ...
  const EXCLUDED_FIELDS = new Set([
    "Match", "Team", "Id", "RecordType",
    "ScouterName", "ScouterError", "Time", "Mode", "DriveStation",
  ]);

  // Metrics excluded from radar chart
  const RADAR_EXCLUDED = new Set([
    "Time", "Drive Station", "Strategy", "Avoidance",
    "LadderLocation", "Id", "StartingLocation",
  ]);
  ...
  const sd = (arr, mu) => Math.sqrt(arr.reduce((s, v) => s + (v - mu) ** 2, 0) / arr.length);
  ...
  function textColorForBg(bg) {
    ...
    const darkColors = ["black", "#000", "#000000", "rgb(0,0,0)", "rgb(0, 0, 0)",
  ...
  function computeGlobalStats(values) {
    ...
    return { mean: mu, sd: sd(values, mu), p25: percentile(values, 25), p50: percentile(values, 50), p75: percentile(values, 75), isNumeric: true };
  }
  ...
  function buildGrid() {
    ...
    if (selectedMetric === OPR_DISPLAY) {
      if (!Object.keys(teamOPRs).length && eventCode) {
        fetchEventOPRs(eventCode).then((oprs) => { teamOPRs = oprs; buildOPRGrid(); });
        return;
      }
      buildOPRGrid();
      return;
    ...
    if (isNumericMetric) {
      ...
      availableTeams.forEach((team) => {
        (teamData[team] ?? []).forEach((row) => {
          ...
          if (val !== 0 && val !== -1 && isNumeric(row[dataMetric])) allValues.push(val);
        });
      });
    ...
    const maxMatchCount = availableTeams.reduce((max, team) => Math.max(max, (teamData[team] ?? []).length), 0);
    ...
    rowData = availableTeams
      ...
      .map((row, i, arr) => {
        if (isNumericMetric && !isBooleanMetric && !isClimbStateMetric && row.mean !== null) {
          ...
          row.alexPercentile = rank < 0.2 ? 0 : rank < 0.4 ? 20 : rank < 0.6 ? 40 : rank < 0.8 ? 60 : 80;
        } else {
  ...
  function buildOPRGrid() {
    ...
    rowData = oprTeams
      .map((team) => ({ team, hasData: true, mean: teamOPRs[team] ?? null, median: teamOPRs[team] ?? null }))
  ...
  function makeTeamColumn() {
    return {
      headerName: "Team", field: "team", pinned: "left", width: 100,
      headerClass: "header-center", cellClass: "cell-center",
      cellStyle: { background: "#C81B00", color: "white", fontWeight: "bold", fontSize: "18px", textAlign: "center" },
    };
  }

  function makeQColumn(q, isNumericMetric, isBooleanMetric, isClimbStateMetric, inverted) {
    return {
  ...
  function statCellStyle(bg, color, extraBorder = null) {
    return { background: bg, color, fontWeight: "bold", fontSize: "18px", textAlign: "center", ...(extraBorder ? { borderLeft: extraBorder } : {}) };
  }
  ...
  function makeMeanColumn(hide, inverted) {
    return {
      ...
      cellStyle: (params) => {
        const v = params.value;
        const bg = (v === null || v === undefined) ? "#4D4D4D" : colorFromStats(v, globalStats, inverted, dataMetric);
  ...
  function updateChartDataset(chart) {
    ...
    const isNumeric = selectedMetric === OPR_DISPLAY ? true : checkIsNumericMetric(dataMetric);
  ...
  function getBarOption(filterSet) {
    ...
    return {
      ...
      series: [{
        data: filtered.map((r) => r.mean), type: "bar", name: selectedMetric,
  ...
  function getLineOption(filterSet) {
    ...
    return {
      ...
      yAxis: { type: "value", name: selectedMetric, axisLabel: axisLabelStyle, nameTextStyle: axisNameStyle },
      series: [{
        data: filtered.map((r) => r.mean), type: "line", name: selectedMetric,
        lineStyle: { color: "#C81B00" }, itemStyle: { color: "#C81B00" },
        label: { show: true, color: "#ffffff", rotate: 90, offset: [20, 0] },
      }],
    };
  }
    ...
    if (numericMetrics.length < 3) {
      return {
        title: {
          ...
          left: "center", top: "center", textStyle: { color: "#fff", fontSize: 14 },
        },
      };
    }
...
<style>
  ...
  .controls {
    ...
    color: white; font-size: 18px; display: flex; gap: 30px; align-items: center; justify-content: center;
  ...
  .plus-btn {
    ...
    font-size: 2rem; font-weight: 600; border: 2px solid var(--frc-190-red);
  ...
  .dropdown {
    ...
    background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%);
    border: 2px solid var(--frc-190-red); border-radius: 8px; list-style: none;
    padding: 0; margin: 0; width: 150px; box-shadow: 0 4px 15px rgba(0,0,0,0.4); z-index: 10; overflow: hidden;
  }
