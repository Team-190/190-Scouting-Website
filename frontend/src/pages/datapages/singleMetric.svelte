<script>
  ...
  async function fetchAllData() {
    ...
    const response = await fetch(
      "http://localhost:8000/allData?eventCode=" + eventCode,
    );
    ...
    const columnDefs = [
      ...
      ...qLabels.map((q) => ({
        ...
        cellStyle: (params) => {
          ...
          const inverted = ["time_of_climb", "climb_time"].includes(
            selectedMetric,
          );
  ...
  function getBarOption(filterSet) {
    ...
    return {
      title: {
        text: `${selectedMetric.replaceAll("_", " ")}`,
          ...
          {#if chart.showFilter}
            <div class="local-filter-panel">
              <div class="local-filter-actions">
                ...
                <button
                  class="mini-btn"
                  on:click={() => deselectChartAll(chart)}>Deselect All</button
                >
              </div>
