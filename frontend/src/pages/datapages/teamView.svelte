<script lang="ts">
  ...
  async function loadTeamData(teamNumber: string) {
    ...
    if (data.length > 0) {
      ...
      for (const match of data) {
        ...
        match.ClimbState = climbData.EndgameClimb;
        match.AutoClimb = climbData.AutoClimb;
        console.log("match " + match.Match + " autoclimb: " + climbData.AutoClimb);
        // IT WORKS WHY IS IT NOT DOING STUFF TO THE GRID AHGWUASDOIFDFJH
      }
    }
  ...
  function buildGrid(matches: any[]) {
    ...
    const rowData = displayMetrics.map((metric) => {
      ...
      row.mean =
        isNumericMetric && !isBool && !isClimb && values.length
          ? Number(mean(values.filter((v) => v !== -1)).toFixed(2))
          : null;
      row.median =
        isNumericMetric && !isBool && !isClimb && values.length
          ? Number(median(values.filter((v) => v !== -1)).toFixed(2))
          : null;
      return row;
    });
