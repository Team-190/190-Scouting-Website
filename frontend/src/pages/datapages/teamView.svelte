<script lang="ts">
  ...
  fetchGracePage(eventCode).then((res) => res.json()).then((data) => { garceData = data; });
  ...
  function getClimbStateColor(climbState, attemptClimb) {
    ...
    if (s === "no_climb" || s === "no climb" || s === "noclimb") {
      ...
      if (a === "yes" || a === "true" || a === "1" || attemptClimb === true || attemptClimb === 1) return "#FF0000";
      return "#000000";
    }
  ...
  function fetchGraceRating(team) {
    if (!garceData || garceData[team] === undefined) return rating[rating.length - 1];
  ...
  function buildGrid(matches) {
    ...
    const displayMetrics = Object.keys(matches[0]).filter((k) => !EXCLUDED_FIELDS.includes(k));
    ...
    displayMetrics.forEach((metric) => {
      ...
      if (isClimbStateMetric) { globalStats[metric] = { mean: 0, sd: 0, isNumeric: false, isClimbState: true }; return; }
      if (isBooleanMetric)    { globalStats[metric] = { mean: 0, sd: 0, isNumeric: false, isBoolean: true }; return; }
    ...
    rowData.forEach((row) => {
      ...
      if (!stats?.isNumeric || !stats.p25 || !stats.p50 || !stats.p75) { row.percentile = null; return; }
    ...
    const statCellStyle = (border: string) => (params) => {
      ...
      if (v === undefined || v === null || v === "" || isBool || isClimb) return { background: "#4D4D4D", color: "white", ...base };
    ...
    setTimeout(() => {
      try {
        domNode.querySelectorAll(".ag-header-cell").forEach((cell) => {
          ...
          const onClick = (e) => { e.stopPropagation(); window.open(fetchTBALink(matchNum), "_blank"); };
  ...
  $: metricOptions = teamViewData?.length > 0
    ? Object.keys(teamViewData[0]).filter((k: string) => !EXCLUDED_FIELDS.includes(k))
    : [];
  ...
  function getBarOption(teamData, metric) {
    return {
      ...
      series: [{
        data: teamData.map((d) => isNumeric(d[metric]) ? Number(d[metric]) : 0),
        type: "bar", name: `Team ${selectedTeam}`,
        itemStyle: { color: "#C81B00" }, label: { show: true, color: "#ffffff" },
      }],
    };
  }
  ...
  function getLineOption(teamData, metric) {
    return {
      ...
      series: [{
        data: teamData.map((d) => isNumeric(d[metric]) ? Number(d[metric]) : 0),
        type: "line", name: `Team ${selectedTeam}`,
        lineStyle: { color: "#C81B00" }, itemStyle: { color: "#C81B00" }, label: { show: true, color: "#ffffff" },
      }],
    };
  }
  ...
  function getRadarOption(teamData) {
    ...
    return {
      tooltip: { trigger: "item" },
      radar: {
        indicator: numericMetrics.map((k, i) => ({ name: METRIC_DISPLAY_NAMES.get(k) || k.replaceAll("_", " "), max: maxValues[i] })),
      },
      series: [{
        type: "radar",
...
<style>
  ...
  button {
    padding: 8px 15px; background: linear-gradient(135deg, #333 0%, #444 100%);
    color: white; font-size: 16px; border: 2px solid var(--frc-190-red);
    border-radius: 6px; cursor: pointer; transition: all 0.3s ease;
  }
