...
<style>
    ...
    @keyframes dropIn {
        0%   { transform: translateY(-200px) scaleX(0.2) rotate(-15deg); opacity: 0; filter: blur(8px); }
        50%  { filter: blur(0px); }
        60%  { transform: translateY(12px) scaleX(1.12) rotate(3deg); opacity: 1; }
        75%  { transform: translateY(-6px) scaleX(0.95) rotate(-2deg); }
        88%  { transform: translateY(3px) scaleX(1.03) rotate(1deg); }
        100% { transform: translateY(0) scaleX(1) rotate(0deg); opacity: 1; }
    }
    ...
    .shockwave {
        ...
        pointer-events: none;
        z-index: 0;
        animation: shockwaveExpand 1.4s ease-out calc(0.2s + var(--ri, 0) * 0.18s) both;
    ...
    .smoke-puff {
        ...
        height: 14px;
        border-radius: 50%;
        background: radial-gradient(circle, rgba(220,220,220,0.85) 0%, transparent 70%);
        pointer-events: none;
        z-index: 3;
        animation: smokeDrift calc(1.3s + var(--si) * 0.12s) ease-out calc(var(--si) * 0.07s) both;
    }

    @keyframes smokeDrift {
        0%   { transform: scale(0.2) translateY(0) translateX(0); opacity: 1; }
        30%  { transform: scale(1.4) translateY(-14px) translateX(calc((var(--si) - 5) * 4px)); opacity: 0.7; }
        100% { transform: scale(3.5) translateY(-60px) translateX(calc((var(--si) - 5) * 12px)); opacity: 0; }
    }
    ...
    .confetti {
        ...
        pointer-events: none;
        z-index: 4;
        animation: confettiBurst calc(1.1s + var(--ci) * 0.03s) ease-out calc(0.05s + var(--ci) * 0.02s) both;
    ...
    @keyframes confettiBurst {
        ...
        100% {
            transform:
                translate(
                    calc(-50% + (cos(calc(var(--ci) * 12.86deg)) * (55px + var(--ci) * 3px))),
                    calc(-50% + (sin(calc(var(--ci) * 12.86deg)) * (55px + var(--ci) * 3px)))
                )
                rotate(900deg)
                scale(0.1);
            opacity: 0;
    ...
    .orbit-ring {
        ...
        border-radius: 50%;
        pointer-events: none;
        animation: orbitSpin calc(2.5s + var(--oi) * 0.7s) linear calc(var(--oi) * 0.4s) infinite;
        z-index: 2;
        transform-origin: center;
    }

    @keyframes orbitSpin {
        from { transform: rotate(calc(var(--oi) * 90deg)); }
        to   { transform: rotate(calc(var(--oi) * 90deg + 360deg)); }
    }
    ...
    .orbit-star {
        ...
        animation: counterSpin calc(2.5s + var(--oi) * 0.7s) linear calc(var(--oi) * 0.4s) infinite;
    }
    ...
    .lightning {
        ...
        color: #ffe066;
        text-shadow: 0 0 10px #fff, 0 0 20px #ffd700, 0 0 40px #ff8800;
        transform-origin: center;
    }
    ...
    .sparkle {
        ...
        font-size: 14px;
        color: gold;
        text-shadow: 0 0 6px gold, 0 0 14px #ffd700, 0 0 22px orange;
        animation: sparklePop calc(1.0s + var(--i) * 0.16s) ease-in-out calc(var(--i) * 0.1s) infinite;
    ...
    @keyframes sparklePop {
        ...
        35%  { opacity: 1;   transform: translate(-50%, -50%) scale(1.5) rotate(60deg); }
        65%  { opacity: 1;   transform: translate(-50%, -50%) scale(1.1) rotate(90deg); }
    ...
    .madness-btn {
        position: relative;
        overflow: hidden;
        background: linear-gradient(135deg, #b8860b 0%, #ffd700 30%, #fffacd 50%, #ffd700 70%, #b8860b 100%);
        ...
        animation: goldShimmer 1.8s linear infinite, glowPulse 1.2s ease-in-out infinite alternate, btnWiggle 3.5s ease-in-out 1.5s infinite;
    ...
    @keyframes glowPulse {
        from {
            box-shadow: 0 0 6px 2px rgba(255,215,0,0.6), 0 0 16px 3px rgba(255,165,0,0.3);
        }
        to {
            box-shadow: 0 0 18px 7px rgba(255,215,0,1), 0 0 36px 12px rgba(255,165,0,0.7), 0 0 60px 20px rgba(255,100,0,0.3);
        }
    }
