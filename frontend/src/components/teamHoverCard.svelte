<script>
  ...
  let colorblindMode = $state(
    COLOR_MODES[_storedMode]
      ? _storedMode
      : (Object.keys(COLOR_MODES)[0] ?? "default"),
  );
  ...
  let displayMetrics = $derived(
    teamMatches.length
      ? Object.keys(teamMatches[0]).filter((k) => !EXCLUDED_FIELDS.has(k))
      : [],
  );
  ...
  let evMetrics = $derived.by(() => {
    ...
    for (const metric of Object.keys(globalStats)) {
      if (!EXCLUDED_FIELDS.has(metric))
        set.add(METRIC_DISPLAY_NAMES.get(metric) ?? metric);
    }
  ...
  let eventRows = $derived.by(() => {
    ...
    const metric =
      userMetric && evMetrics.includes(userMetric)
        ? userMetric
        : (evMetrics[0] ?? "");
    ...
    return [...rows]
      ...
      .map((r) => ({
        ...
        percentile:
          r.mean !== null
            ? computePercentileBucket(r.mean, allMeans, inv)
            : null,
      }));
  });
  ...
  function resolveDataKey(display) {
    for (const [k, v] of METRIC_DISPLAY_NAMES) {
      if (v === display) return k;
    }
    return display;
  }
  function computePercentileBucket(val, allVals, inv = false) {
    const s = [...allVals].sort((a, b) => a - b);
    const p25 = percentile(s, 25),
      p50 = percentile(s, 50),
      p75 = percentile(s, 75);
    if (inv)
      return val <= p25
  ...
  function getCellBg(val, metric) {
    ...
    const inv = INVERTED_METRICS.includes(metric);
    const mode =
      COLOR_MODES[colorblindMode] ??
      COLOR_MODES["default"] ??
      COLOR_MODES[Object.keys(COLOR_MODES)[0]];
    const { p25, p50, p75 } = stats;
                    ...
                    {#each row.matches as val}
                      <td
                        style="background:{getCellBg(
                          val,
                          row.metric,
                        )}; color:{textColorForBg(getCellBg(val, row.metric))};"
                      >
      ...
      <section class="section">
        <div class="section-label-row">
          ...
          <div class="ev-metric-row">
            ...
            <select
              id="hc-metric"
              value={selectedEvMetric}
              onchange={(e) => {
                userMetric = e.currentTarget.value;
              }}
            >
              ...
              <tbody>
                {#each eventRows as row, i}
                  <tr class:highlight={row.isCurrentTeam}>
                    ...
                    <td class="team-cell" class:current-team={row.isCurrentTeam}
                      >{row.team}</td
                    >
                    {#if row.mean !== null}
                      <td
                        style="background:{getCellBg(
                          row.mean,
                          resolveDataKey(selectedEvMetric),
                        )}; color:{textColorForBg(
                          getCellBg(row.mean, resolveDataKey(selectedEvMetric)),
                        )};"
                      >
...
<style>
  ...
  .mini-grid thead th.metric-th {
    text-align: left;
    min-width: 85px;
    max-width: 115px;
  }
  .mini-grid thead th.stat-th {
    min-width: 36px;
  }
  .mini-grid tbody tr:hover {
  ...
  .sep-left {
    border-left: 2px solid #c81b00 !important;
  }
  .sep-thin {
